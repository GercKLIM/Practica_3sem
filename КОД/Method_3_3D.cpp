...
void Method_3_3D() {
	...
	// Запускаем Метод №3
	cout << "Метод ближней точки и нормали в пространстве:" << endl;
	for (int i = 0; i < n; i++) {
...
bool met3_3D(const coord3D& Point, const vector<coord3D>& Poligon, int n) {
	double A, B, C, D;
	coord3D V1, V2;
	...
	return met3(Projection_Point, Projection_Polygon, n);
}